...
class AppFiltroPDF:
    def __init__(self, root):
        self.root = root
        self.root.title("Filtro PDF para Excel | Emanuel")
        ...
        self.cor_azul = "#2563EB"
        self.cor_azul_escuro = "#020617"
        self.cor_verde = "#16A34A"
        self.cor_cinza = "#94A3B8"
        self.cor_fundo = "#0F172A"
        self.cor_caixa = "#111827"
        self.cor_laranja = "#F59E0B"
        self.cor_borda = "#334155"
        self.cor_texto = "#E5E7EB"
    ...
    def configurar_estilos(self):
        try:
            ...
            style.configure(
                "Emanuel.Horizontal.TProgressbar",
                troughcolor="#1E293B",
                background=self.cor_verde,
                bordercolor="#1E293B",
                lightcolor=self.cor_verde,
                darkcolor=self.cor_verde,
                thickness=16,
            )
        except Exception:
            pass

    def criar_botao(self, pai, texto, comando, cor, largura=None):
        botao = tk.Button(
            pai,
            text=texto,
            command=comando,
    ...
    def criar_card(self, pai, titulo, subtitulo, numero=None, cor_destaque=None):
        cor_destaque = cor_destaque or self.cor_azul
        card = tk.Frame(
            pai,
            bg=self.cor_caixa,
        ...
        cabecalho = tk.Frame(card, bg=self.cor_caixa)
        ...
        if numero:
            ...
            badge.grid(row=0, column=0, sticky="nw", padx=(0, 12))

        textos = tk.Frame(cabecalho, bg=self.cor_caixa)
        textos.grid(row=0, column=1, sticky="ew")
        tk.Label(
            textos,
            text=titulo,
            font=("Arial", 16, "bold"),
    ...
    def adicionar_campo_arquivo(self, card, linha, label, variavel, texto_botao, comando):
        tk.Label(
            card,
            text=label,
            font=("Arial", 11, "bold"),
            fg=self.cor_texto,
            bg=self.cor_caixa
        ).grid(row=linha, column=0, sticky="w", padx=(0, 10), pady=(0, 6))
        ...
        campo.grid(row=linha + 1, column=0, sticky="ew", padx=(0, 10), pady=(0, 14), ipady=9)

        botao = self.criar_botao(card, texto_botao, comando, self.cor_azul, largura=16)
        botao.grid(row=linha + 1, column=1, sticky="ew", pady=(0, 14))
        return campo, botao

    def montar_tela(self):
        header = tk.Frame(self.root, bg=self.cor_azul_escuro, height=128)
        header.pack(fill="x")
        header.pack_propagate(False)

        header_conteudo = tk.Frame(header, bg=self.cor_azul_escuro)
        header_conteudo.pack(fill="both", expand=True, padx=30, pady=22)

        tk.Label(
            header_conteudo,
            text="Filtro PDF para Excel",
            font=("Arial", 27, "bold"),
            fg="white",
            bg=self.cor_azul_escuro
        ).pack(anchor="w")

        tk.Label(
            header_conteudo,
            text="Gere as abas do repasse, aplique o segundo filtro e crie a Prioridade de Cobrança em poucos passos.",
            font=("Arial", 12),
            fg="#CBD5E1",
            bg=self.cor_azul_escuro
        ).pack(anchor="w", pady=(6, 0))

        conteudo = tk.Frame(self.root, bg=self.cor_fundo)
        ...
        frame1 = self.criar_card(
            conteudo,
            "Gerar Excel a partir do PDF",
            "Use o relatório original em PDF para criar as abas de conferência, AJ, rescisão e contratos em atraso.",
            numero="1",
            cor_destaque=self.cor_azul
        )
        frame1.grid(row=0, column=0, sticky="nsew", padx=(0, 10), pady=(0, 16))
        self.adicionar_campo_arquivo(frame1, 1, "Arquivo PDF", self.pdf_var, "Buscar PDF", self.buscar_pdf)
        self.adicionar_campo_arquivo(frame1, 3, "Salvar Excel em", self.excel_var, "Escolher local", self.escolher_excel)

        frame2 = self.criar_card(
            conteudo,
            "Atualizar e criar Prioridade de Cobrança",
            "Depois de ajustar as garantias no Excel, gere o arquivo final com a aba Prioridade de Cobrança.",
            numero="2",
            cor_destaque=self.cor_laranja
        )
        frame2.grid(row=0, column=1, sticky="nsew", padx=(10, 0), pady=(0, 16))
        self.adicionar_campo_arquivo(frame2, 1, "Arquivo Excel editado", self.excel_entrada_var, "Buscar Excel", self.buscar_excel_editado)
        self.adicionar_campo_arquivo(frame2, 3, "Salvar novo Excel em", self.excel_saida_var, "Escolher local", self.escolher_excel_saida)

        acoes = tk.Frame(conteudo, bg=self.cor_fundo)
        acoes.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(0, 16))
        ...
        self.btn_processar = self.criar_botao(acoes, "1. Processar PDF", self.processar_pdf_thread, self.cor_verde)
        self.btn_processar.grid(row=0, column=0, sticky="ew", padx=(0, 8))

        self.btn_conferencia = self.criar_botao(acoes, "2. Aplicar segundo filtro", self.atualizar_conferencia_thread, self.cor_laranja)
        self.btn_conferencia.grid(row=0, column=1, sticky="ew", padx=8)

        self.btn_abrir = self.criar_botao(acoes, "Abrir Excel gerado", self.abrir_excel, self.cor_cinza)
        self.btn_abrir.grid(row=0, column=2, sticky="ew", padx=8)

        self.btn_admin = self.criar_botao(acoes, "Área ADM", lambda: exibir_area_admin(self.root), "#334155")
        self.btn_admin.grid(row=0, column=3, sticky="ew", padx=(8, 0))
        ...
        painel.grid(row=2, column=0, columnspan=2, sticky="nsew")
        painel.grid_columnconfigure(0, weight=1)
        painel.grid_rowconfigure(4, weight=1)

        modulos = tk.Frame(painel, bg=self.cor_caixa)
        modulos.grid(row=0, column=0, sticky="ew", pady=(0, 12))
        modulos.grid_columnconfigure(1, weight=1)

        self.btn_anomalias = self.criar_botao(
            modulos,
            "Anomalias / Reajustes e Seguros",
            self.abrir_modulo_anomalias,
            "#0891B2"
        )
        self.btn_anomalias.grid(row=0, column=0, sticky="w", padx=(0, 14))

        tk.Label(
            modulos,
            text="Importe o relatório mensal, copie contratos e acesse o Zimbra.",
            font=("Arial", 11),
            fg=self.cor_texto_suave,
            bg=self.cor_caixa
        ...
        self.status.grid(row=1, column=0, sticky="w", pady=(0, 8))

        progress_area = tk.Frame(painel, bg=self.cor_caixa)
        progress_area.grid(row=2, column=0, sticky="ew", pady=(0, 16))
        progress_area.grid_columnconfigure(0, weight=1)
        self.progress_bar = ttk.Progressbar(
            progress_area,
            variable=self.progress_var,
            maximum=100,
            style="Emanuel.Horizontal.TProgressbar"
        )
        ...
        log_frame = tk.Frame(painel, bg="#0F172A")
    ...
    def set_progress(self, valor):
        ...
        self.root.after(0, lambda: [
            ...
            self.progress_label.config(text=f"{valor}%")
        ])
...
class JanelaAnomalias(tk.Toplevel):
    ...
    def montar_tela(self):
        ...
        linha_pdf = tk.Frame(self, bg="#111827", highlightbackground="#334155", highlightthickness=1)
        ...
        tk.Button(
            ...
            font=("Arial", 11, "bold"), bg="#DC2626", fg="white", padx=12, pady=8
        ...
        self.label_pdf = tk.Label(linha_pdf, text=texto_pdf, bg="#111827", fg="#94A3B8", font=("Arial", 11))
        ...
        acoes = tk.Frame(self, bg="#0F172A")
        acoes.pack(fill="x", padx=18, pady=(0, 10))

        tk.Label(acoes, text="Categoria:", font=("Arial", 11, "bold"), bg="#0F172A", fg=self.cor_azul).pack(side="left")
        ...
        tk.Label(acoes, text="Competência:", font=("Arial", 11, "bold"), bg="#0F172A", fg=self.cor_azul).pack(side="left")
        ...
        self.aba_geral = tk.Frame(self.abas_anomalias, bg="#0F172A")
        self.aba_renovacao = tk.Frame(self.abas_anomalias, bg="#0F172A")
        self.aba_finalizados = tk.Frame(self.abas_anomalias, bg="#0F172A")
        ...
        area_tabela = tk.Frame(self.aba_geral, bg="#0F172A")
        ...
        cabecalho_renovacao = tk.Frame(self.aba_renovacao, bg="#111827", highlightbackground="#334155", highlightthickness=1)
        ...
        area_renovacao = tk.Frame(self.aba_renovacao, bg="#0F172A")
        ...
        cabecalho_finalizados = tk.Frame(self.aba_finalizados, bg="#111827", highlightbackground="#334155", highlightthickness=1)
        ...
        area_finalizados = tk.Frame(self.aba_finalizados, bg="#0F172A")
        ...
        email = tk.Frame(self, bg="#111827", highlightbackground="#334155", highlightthickness=1)
        ...
        tk.Label(
            ...
            font=("Arial", 11, "bold"), bg="#111827", fg=self.cor_azul
        ...
        self.botao(email, "Alterar link Zimbra", self.redefinir_link_zimbra, "#475569")

        tk.Label(self, textvariable=self.status_var, font=("Arial", 11, "bold"), bg="#0F172A", fg=self.cor_azul).pack(
    ...
    def criar_card(self, pai, titulo):
        card = tk.Frame(pai, bg="#111827", highlightbackground="#334155", highlightthickness=1)
        card.pack(side="left", fill="x", expand=True, padx=(0, 8))
        tk.Label(card, text=titulo, font=("Arial", 10), fg="#94A3B8", bg="#111827").pack(anchor="w", padx=12, pady=(8, 1))
        valor = tk.Label(card, text="0", font=("Arial", 18, "bold"), fg=self.cor_azul, bg="#111827")
...
def exibir_area_admin(root):
    ...
    janela_senha.configure(bg="#0F172A")
    ...
    frame = tk.Frame(janela_senha, bg="#0F172A")
    frame.pack(fill="x", padx=28)

    tk.Label(frame, text="Senha ADM", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
    ...
    frame_texto = tk.Frame(janela, bg="#0F172A")
    frame_texto.pack(fill="both", expand=True, padx=18, pady=(0, 10))

    texto = tk.Text(frame_texto, font=("Consolas", 10), bg="#111827", fg="#E5E7EB", wrap="none")
    ...
    botoes = tk.Frame(janela, bg="#0F172A")
...
"""
    ...
    frame_campos = tk.Frame(janela, bg="#0F172A")
    frame_campos.pack(fill="x", padx=22, pady=(5, 5))

    tk.Label(frame_campos, text="Nome completo", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
    tk.Entry(frame_campos, textvariable=nome_var, font=("Arial", 12)).pack(fill="x", pady=(2, 8), ipady=5)

    tk.Label(frame_campos, text="E-mail", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
    tk.Entry(frame_campos, textvariable=email_var, font=("Arial", 12)).pack(fill="x", pady=(2, 8), ipady=5)

    tk.Label(frame_campos, text="Senha", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
    tk.Entry(frame_campos, textvariable=senha_var, font=("Arial", 12), show="*").pack(fill="x", pady=(2, 8), ipady=5)

    frame_botoes = tk.Frame(janela, bg="#0F172A")
...
def exibir_login_usuario(root, config_salva=None):
    ...
    tk.Label(frame, text="E-mail", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
    tk.Entry(frame, textvariable=email_var, font=("Arial", 12)).pack(fill="x", pady=(2, 10), ipady=5)

    tk.Label(frame, text="Senha", font=("Arial", 11, "bold"), bg="#0F172A", fg="#2563EB").pack(anchor="w")
...
class MenuModulos:
    def __init__(self, root):
        self.root = root
        self.root.title("Central de Ferramentas | Emanuel")
    ...
    def montar_tela(self):
        topo = tk.Frame(self.root, bg=self.cor_azul_escuro, height=160)
        topo.pack(fill="x")
        topo.pack_propagate(False)

        conteudo_topo = tk.Frame(topo, bg=self.cor_azul_escuro)
        conteudo_topo.pack(fill="both", expand=True, padx=46, pady=28)
        ...
        tk.Label(
            conteudo_topo,
            text="Escolha o módulo que deseja utilizar para executar sua rotina com mais agilidade.",
            font=("Arial", 12),
            fg="#CBD5E1",
            bg=self.cor_azul_escuro
        ...
        corpo.pack(fill="both", expand=True, padx=46, pady=34)
        ...
        self.criar_card_modulo(
            ...
            descricao="Processar repasses, contratos AJ, rescisões, 1º aluguel e prioridade de cobrança.",
            cor_botao=self.cor_azul,
            comando=self.abrir_filtro_pdf
        ).grid(row=0, column=0, sticky="nsew", padx=(0, 14))
        ...
        self.criar_card_modulo(
            ...
            descricao="Conferir reajustes, seguros, copiar contratos e abrir o Zimbra com mais praticidade.",
            cor_botao=self.cor_anomalias,
            comando=self.abrir_anomalias
        ).grid(row=0, column=1, sticky="nsew", padx=14)
        ...
        self.criar_card_modulo(
            ...
            etiqueta="Restrito",
            titulo="Área ADM",
            descricao="Consultar registros de acesso, usuários e exportar relatórios administrativos.",
            cor_botao="#334155",
            comando=self.abrir_area_admin
        ).grid(row=0, column=2, sticky="nsew", padx=(14, 0))

        rodape = tk.Frame(self.root, bg="#111827", height=64, highlightbackground=self.cor_borda, highlightthickness=1)
        rodape.pack(fill="x", side="bottom")
        rodape.pack_propagate(False)
        tk.Label(
            rodape,
            text="Emanuel • Central interna de apoio operacional",
            font=("Arial", 11),
            fg=self.cor_texto_suave,
            bg="#111827"
        ).pack(anchor="w", padx=46, pady=20)

    def criar_card_modulo(self, pai, icone, etiqueta, titulo, descricao, cor_botao, comando):
        card = tk.Frame(
            pai,
            bg=self.cor_caixa,
            highlightbackground=self.cor_borda,
        ...
        topo_card = tk.Frame(card, bg=self.cor_caixa)
